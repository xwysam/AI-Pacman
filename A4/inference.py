        ...
        * When a ghost is captured by Pacman, all beliefs should be
        ...
        if noisyDistance != None:
            ...
            for p in self.legalPositions:
                distence = util.manhattanDistance(p, pacmanPosition)

                if emissionModel[distence] > 0:
                    belief[p] = emissionModel[distence] * self.beliefs[p]
        else:
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for nxtP, prob in newPosition.items():
                belief[nxtP] += prob * self.beliefs[p]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(len(legalPositions)):
            ...
            self.beliefs[legalPositions[i]] = float(num) / float(numParticles)
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance != None:
            ...
            for position in self.particles:
                distence = util.manhattanDistance(pacmanPosition,position)
                prob = emissionModel[distence]
                belif[position] += prob
            total_prob = belif.totalCount()

            if total_prob != 0 :
                pos_lst = []

                for i in range(self.numParticles):
                    pos_lst.append(util.sample(belif))
                self.particles = pos_lst
    ...
    def elapseTime(self, gameState):
        ...
        for position in self.particles:
            ...
            belif = self.getPositionDistribution(ghostPosition)
            point = util.sample(belif)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for i in itertools.product(self.legalPositions,repeat=numGhost):
            pos_list.append(i)
        random.shuffle(pos_list)

        for i in range(len(pos_list)):
            num = numParticles/len(pos_list)
            for j in range(num):
                Positons.append(pos_list[i])
        self.particles = Positons
        ...
        for position in self.particles:
            ...
            for i in range(numGhost):
                ...
                if noisyDistances[i] != None:
                    distence = util.manhattanDistance(pacmanPosition,position[i])
                    prb = prb * emissionModels[i][distence]

                else:
                    position = self.getParticleWithGhostInJail(position,i)

            belif[position] += prb

        if belif.totalCount() != 0:
            belif.normalize()

            for i in range(numParticles):
                lst_Position.append(util.sample(belif))

            self.particles = lst_Position
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(numGhost):
                ...
                ghostPos = setGhostPositions(gameState,newParticle)
                newDistence = getPositionDistributionForGhost(ghostPos,i,agent)
                newParticle[i] = util.sample(newDistence)
